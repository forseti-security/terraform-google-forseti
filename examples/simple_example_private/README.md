...
This example illustrates how to set up a minimal Forseti installation without public IP's on the Server and Client VM's or the CloudSQL instance.  Both the Server and Client VM have outbound public Internet access via a Cloud NAT.  The Server VM has access to the GCP API's via the Cloud NAT as well.

SSH is achieved through the [SSH over GCP Identity Aware Proxy](https://cloud.google.com/iap/docs/using-tcp-forwarding#starting_ssh).  No further configuration is required beyond the execution of this example.
