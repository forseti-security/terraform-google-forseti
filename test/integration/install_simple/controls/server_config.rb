...
control "server-config" do
  title "Forseti server config yaml"
  ...
  describe file("/home/ubuntu/forseti-security/configs/forseti_conf_server.yaml") do
    ...
    describe "scanner" do
      ...
      it "configures bigquery_enabled" do
        expect(config["scanner"]["scanners"]).to include("name" => "bigquery", "enabled" => false)
      end
      ...
      it "configures cloudsql_acl_enabled" do
        expect(config["scanner"]["scanners"]).to include("name" => "cloudsql_acl", "enabled" => false)
      end

      it "configures config_validator_enabled" do
        expect(config["scanner"]["scanners"]).to include("name" => "config_validator", "enabled" => true, "verify_policy_library" => true)
      end
      ...
      it "configures firewall_rule_enabled" do
        expect(config["scanner"]["scanners"]).to include("name" => "firewall_rule", "enabled" => false)
      end
      ...
      it "configures kms_scanner_enabled" do
        expect(config["scanner"]["scanners"]).to include("name" => "kms_scanner", "enabled" => false)
      end
      ...
      it "configures service_account_key_enabled" do
        expect(config["scanner"]["scanners"]).to include("name" => "service_account_key", "enabled" => false)
      end
    end
