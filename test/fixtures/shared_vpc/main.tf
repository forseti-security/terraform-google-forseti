...
resource "null_resource" "ssh_server_iap_tunnel" {
  ...
  provisioner "local-exec" {
    command =  "nohup gcloud compute start-iap-tunnel ${module.forseti-shared-vpc.forseti-server-vm-name} 22 --local-host-port=localhost:2222 --zone=${var.region}-c &"
  }
...
resource "null_resource" "ssh_client_iap_tunnel" {
  ...
  provisioner "local-exec" {
    command = "nohup gcloud compute start-iap-tunnel ${module.forseti-shared-vpc.forseti-client-vm-name} 22 --local-host-port=localhost:2223 --zone=${var.region}-c &"
  }
