...
resource "null_resource" "ssh_iap_tunnel" {
  ...
  provisioner "remote-exec" {
    inline = [
      "nohup gcloud compute start-iap-tunnel ${module.forseti-shared-vpc.forseti-server-vm-name} 22 --local-host-port=localhost:22 &",
      "nohup gcloud compute start-iap-tunnel ${module.forseti-shared-vpc.forseti-server-vm-name} 22 --local-host-port=localhost:23 &",
    ]
