...
module "bastion" {
  ...
  key_suffix = "_server_monitoring_enabled"
}

#-------------------------#
# Forseti
#-------------------------#
module "forseti-server-monitoring-enabled" {
  source = "../../../"
...
resource "google_compute_firewall" "forseti_bastion_to_vm" {
  name                    = "forseti-bastion-to-vm-ssh-${module.forseti-server-monitoring-enabled.suffix}"
  project                 = var.project_id
  network                 = var.network
  target_service_accounts = [module.forseti-server-monitoring-enabled.forseti-server-service-account]
...
resource "null_resource" "wait_for_server" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type                = "ssh"
      user                = "ubuntu"
      host                = module.forseti-server-monitoring-enabled.forseti-server-vm-ip
